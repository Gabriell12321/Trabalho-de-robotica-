...
int JoyX;
int JoyY;
/* Variáveis que armazenarão os estados do botões.     */
int Estado_bt_A;
int Estado_bt_B;
int Estado_bt_C;
int Estado_bt_D;
...
int angEsq;
int angGuarra;
/* Cria os objetos de controle para casa servo.        */
...
void setup() {
  ...
  /* servos, chamando a função auxiliar angulo() que   */
  /* irá converter o ângulo para duração de pulsos, já */
  /* que a biblioteca ServoTimer2 não trabalha com     */
  /* ângulos e sim com duração de pulsos elétrico.     */
  angBase = angulo(90);
  angDir = angulo(90);
  angEsq = angulo(90);
  angGuarra = angulo(90);
...
void loop() {
  /* Chama a função auxiliar para realizar a leitura   */
  /* do Joystick e botões armazendo os dados nas       */
  ...
  switch (Escolha) {
    ...
    case 'A':
      ...
      if (JoyX > 700 && angGuarra < angulo(100)) {
        ...
        /* Aguarda 5  milissegundos para atualizar a   */
        /* posição.                                    */
        delay(5);
      }
...
/* Função auciliar para realizar a leitura do Joystick.*/
void Ler_Joystick() {
